(function (global, factory) {
  ...
  const useInteractions = function (propsList) {
    ...
    const getReferenceProps = React__namespace.useCallback(userProps => mergeProps(userProps, propsList, 'reference'), // eslint-disable-next-line react-hooks/exhaustive-deps
    deps);
    const getFloatingProps = React__namespace.useCallback(userProps => mergeProps(userProps, propsList, 'floating'), // eslint-disable-next-line react-hooks/exhaustive-deps
    deps);
    const getItemProps = React__namespace.useCallback(userProps => mergeProps(userProps, propsList, 'item'), // eslint-disable-next-line react-hooks/exhaustive-deps
    deps);
  ...
  const FloatingPortal = _ref => {
    ...
    const [modal, setModal] = React__namespace.useState(true);
    ...
    const managerRef = React__namespace.useRef(null);
    const renderGuards = !!children && !!(root || portalNode) && preserveTabOrder && !modal;
    ...
    }, renderGuards && /*#__PURE__*/React__namespace.createElement(FocusGuard, {
      ref: beforeOutsideRef,
      onFocus: () => {
        var _managerRef$current;

        (_managerRef$current = managerRef.current) == null ? void 0 : _managerRef$current.handleBeforeOutside();
      }
    }), renderGuards && /*#__PURE__*/React__namespace.createElement("span", {
      "aria-owns": portalNode == null ? void 0 : portalNode.id,
      style: HIDDEN_STYLES
    }), root ? /*#__PURE__*/reactDom$1.createPortal(children, root) : portalNode ? /*#__PURE__*/reactDom$1.createPortal(children, portalNode) : null, renderGuards && /*#__PURE__*/React__namespace.createElement(FocusGuard, {
      ref: afterOutsideRef,
      onFocus: () => {
        var _managerRef$current2;

        (_managerRef$current2 = managerRef.current) == null ? void 0 : _managerRef$current2.handleAfterOutside();
      }
    }));
  };
  ...
  */
  ...
  function FloatingFocusManager(_ref) {
    ...
    const didFocusOutRef = React__namespace.useRef(false);
    ...
    const getTabbableElements = React__namespace.useCallback(container => {
      ...
      let orderArr = orderRef.current; // Make the floating element focusable if it has no tabbable content
    ...
    React__namespace.useEffect(() => {
      ...
      function onFocusOut(event) {
        var _refs$floating$curren, _getAncestors;

        didFocusOutRef.current = false;
        const relatedTarget = event.relatedTarget;

        if (relatedTarget == null || relatedTarget === (portalContext == null ? void 0 : portalContext.beforeOutsideRef.current) || relatedTarget === (portalContext == null ? void 0 : portalContext.afterOutsideRef.current)) {
          return;
        }

        if (relatedTarget === (portalContext == null ? void 0 : portalContext.beforeInsideRef.current) || relatedTarget === (portalContext == null ? void 0 : portalContext.afterInsideRef.current)) {
          didFocusOutRef.current = true;
          return;
        }

        const focusMovedOutsideFloating = !((_refs$floating$curren = refs.floating.current) != null && _refs$floating$curren.contains(relatedTarget));
    ...
    }, [nodeId, tree, modal, onOpenChange, orderRef, dataRef, getTabbableElements, refs, portalContext]);
    React__namespace.useEffect(() => {
      const floating = refs.floating.current;
    ...
    }, [modal, guards, refs]); // Synchronize the `modal` value to the FloatingPortal context. It will
    // decide whether or not it needs to render its own guards.

    index(() => {
      portalContext == null ? void 0 : portalContext.setModal(modal);
    }, [portalContext, modal]);
    React__namespace.useImperativeHandle(portalContext == null ? void 0 : portalContext.managerRef, () => ({
      handleBeforeOutside() {
        const els = getTabbableElements().filter(el => el !== refs.domReference.current);
        focus(els[0]);
      },
    ...
    return /*#__PURE__*/React__namespace.createElement(React__namespace.Fragment, null, renderGuards && /*#__PURE__*/React__namespace.createElement(FocusGuard, {
      ...
      onFocus: event => {
        ...
        } else if (portalContext != null && portalContext.preserveTabOrder) {
          const els = getTabbableElements(getDocument(refs.floating.current).body);
  ...
  const useListNavigation = function (_ref, _temp2) {
    ...
    }, [enabled, open, selectedIndex, listRef, focusItem, onNavigate]); // Sync `activeIndex` to be the focused item while the floating element is
    // open.
    ...
    }, [enabled, open, activeIndex, selectedIndex, nested, listRef, allowEscape, orientation, rtl, virtual, onNavigate, focusItem, disabledIndicesRef]); // Ensure the parent floating element has focus when a nested child closes
      ...
      return {
        ...
        floating: {
          ...
          onBlur(event) {
            ...
            const lostFocusToGuard = ((_event$relatedTarget = event.relatedTarget) == null ? void 0 : _event$relatedTarget.getAttribute('data-floating-ui-focus-guard')) != null; // shift+tab back to the reference element should unset the
            // `activeIndex`.
